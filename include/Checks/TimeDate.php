...
{	
    protected function createChecks()
    {
			...
			$this->checks = array(
						new TimeDate_Checker(TT_WORDPRESS, ERRORLEVEL_ERROR, __all('Use of date_i18n()'), '/\sdate_i18n\s?\(\s?["|\'][A-Za-z\s]+["|\']\s?\)/', 'ut_timedate_date_i18n.zip'),
						new TimeDate_Checker(TT_WORDPRESS, ERRORLEVEL_ERROR, __all('Use of the_date()'), '/[^get_]the_date\s?\(\s?["|\'][A-Za-z\s]+["|\']\s?\)/', 'ut_timedate_the_date.zip'),
						new TimeDate_Checker(TT_WORDPRESS, ERRORLEVEL_ERROR, __all('Use of the_time()'), '/[^get_]the_time\s?\(\s?["|\'][A-Za-z\s]+["|\']\s?\)/', 'ut_timedate_the_time.zip'),
			);
    }
}