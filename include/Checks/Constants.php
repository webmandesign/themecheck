    ...
    {		
		...
		foreach ( $php_files as $php_key => $phpfile ) {
            if ( preg_match( '/[\s|]' . $this->code[0] . '/', $phpfile, $matches ) ) {
                ...
                $this->messages[] = __all('Constant <strong>%1$s</strong> was found in the file <strong>%2$s</strong>. Use <strong>%3$s</strong> instead. %4$s', esc_html($errori), esc_html($filename), esc_html($this->code[1]), $grep );
