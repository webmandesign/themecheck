        ...
        foreach ( $php_files as $php_key => $phpfile ) {
            if ( preg_match( $this->code, $phpfile, $matches ) ) {
                if ( !strpos( $php, 'get_post_format' ) && !strpos( $php, 'has_post_format' ) ) {
                    ...
                    $this->messages[] = sprintf(__('<strong>add_theme_support()</strong> was found in the file <strong>%1$s</strong>. However get_post_format and/or has_post_format were not found%2$s.'), $filename, $css_found);
...
{	
    ...
    {
			$this->title = __("Registration of theme features");
			$this->checks = array(
						new PostFormat_Checker(TT_WORDPRESS, ERRORLEVEL_ERROR, __("Implementation of add_theme_support()"), '/add_theme_support\(\s?("|\')post-formats(,*)?("|\')/m', 'ut_postformat.zip'),
			);
    }
}