...
class AdminMenu extends Check
{
		protected function createChecks()
    {
			...
			$this->checks = array(
						...
						new AdminMenu_Checker('ADMIN_USERLEVELS1', TT_WORDPRESS | TT_WORDPRESS_CHILD, ERRORLEVEL_WARNING, __all('Detection of user levels that were deprecated in Worpress 2.0. <a href="https://codex.wordpress.org/Roles_and_Capabilities">Wordpress codex</a>'),'/([^_](add_(admin|submenu|menu|dashboard|posts|media|links|pages|comments|theme|plugins|users|management|options)_page)\s?\([^,]*,[^,]*,\s[\'|"]?(level_[0-9]|[0-9])[^;|\r|\r\n]*)/', 'ut_adminmenu_addmenupage.zip'),
						new AdminMenu_Checker('ADMIN_USERLEVELS2', TT_WORDPRESS | TT_WORDPRESS_CHILD, ERRORLEVEL_WARNING, __all('Detection of user levels that were deprecated in Worpress 2.0. <a href="https://codex.wordpress.org/Roles_and_Capabilities">Wordpress codex</a>'), '/[^a-z0-9](current_user_can\s?\(\s?[\'\"]level_[0-9][\'\"]\s?\))[^\r|\r\n]*/', 'ut_adminmenu_current_user_can.zip'),
			);
    }
}