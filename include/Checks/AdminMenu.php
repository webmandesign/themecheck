...
class AdminMenu extends Check
{
		protected function createChecks()
    {
			...
			$this->checks = array(
						new AdminMenu_Checker(TT_WORDPRESS | TT_WORDPRESS_CHILD, ERRORLEVEL_CRITICAL, __all('Themes that support admin pages should use <strong>add_theme_page()</strong> instead of other functions (add_admin_page, add_submenu_page...)'), '/([^_]add__all(admin|submenu|menu|dashboard|posts|media|links|pages|comments|plugins|users|management|options)_page\()/', 'ut_adminmenu_addadminpage.zip'),
						new AdminMenu_Checker(TT_WORDPRESS | TT_WORDPRESS_CHILD, ERRORLEVEL_WARNING, __all('Detection of user levels that were deprecated in Worpress 2.0. <a href="http://codex.wordpress.org/Roles_and_Capabilities">Wordpress codex</a>'), '/([^_](add__all(admin|submenu|menu|dashboard|posts|media|links|pages|comments|theme|plugins|users|management|options)_page)\s?\([^,]*,[^,]*,\s[\'|"]?(level_[0-9]|[0-9])[^;|\r|\r\n]*)/', 'ut_adminmenu_addmenupage.zip'),
						new AdminMenu_Checker(TT_WORDPRESS | TT_WORDPRESS_CHILD, ERRORLEVEL_WARNING, __all('Detection of user levels that were deprecated in Worpress 2.0. <a href="http://codex.wordpress.org/Roles_and_Capabilities">Wordpress codex</a>'), '/[^a-z0-9](current_user_can\s?\(\s?[\'\"]level_[0-9][\'\"]\s?\))[^\r|\r\n]*/', 'ut_adminmenu_current_user_can.zip'),
			);
    }
}